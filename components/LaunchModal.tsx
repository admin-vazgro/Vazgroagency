...
const inputClass =
  "w-full bg-white border border-[#D8D8D8] text-[#0A0A0A] font-ibm-mono text-[14px] tracking-[0.5px] px-4 py-3 focus:outline-none focus:border-[#D6E264] transition-colors placeholder:text-[#777777]";

const labelClass = "font-ibm-mono text-[14px] font-normal text-[#666666] tracking-[1.5px] md:tracking-[2px] block mb-2";
...
export default function LaunchModal({ pkg, onClose }: Props) {
  ...
  return (
    ...
    >
      <div className="flex max-h-[95dvh] w-full flex-col overflow-hidden border border-[#D8D8D8] bg-[#F5F5F5] sm:max-w-[560px]">

        {/* Header */}
        <div className="shrink-0 border-b border-[#E5E5E5] px-4 py-4 sm:px-6">
          <div className="flex items-start justify-between gap-4">
            <div className="min-w-0">
              <span className="font-ibm-mono text-[14px] text-[#777777] tracking-[1.5px] md:tracking-[2px] break-words">⚡ LAUNCH · {pkg.name.toUpperCase()}</span>
              <div className="mt-1 flex flex-wrap items-center gap-3">
                <span className="font-grotesk text-[24px] sm:text-[28px] font-normal text-[#0A0A0A] tracking-[-1px] leading-none">
                  £{pkg.price.toLocaleString()}
                </span>
                <span className="font-ibm-mono text-[14px] text-[#666666] tracking-[1px] border border-[#D8D8D8] px-2 py-1">
            ...
            <button
              onClick={onClose}
              className="w-9 h-9 flex items-center justify-center border border-[#D8D8D8] hover:border-[#555] text-[#666666] hover:text-[#0A0A0A] transition-colors cursor-pointer bg-transparent"
              aria-label="Close"
            >
        ...
        <div className="flex border-b border-[#E5E5E5] shrink-0">
          {steps.map((s) => (
            <div
              key={s.n}
              className="flex-1 py-3 text-center border-r last:border-r-0 border-[#E5E5E5] transition-colors"
              style={{
                background: step === s.n ? "#D6E264" : step > s.n ? "#0F0F0F" : "#0A0A0A",
              }}
            >
              <span
                className="font-ibm-mono text-[14px] font-normal tracking-[1px] md:tracking-[2px]"
        ...
        {/* Error banner */}
        {error && (
          <div className="shrink-0 border-b border-[#FF4444] bg-[#FFF0F0] px-5 py-3">
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6">
          ...
          {step === 1 && (
            <div className="flex flex-col gap-4">
              <h3 className="font-grotesk text-[18px] font-normal text-[#0A0A0A] tracking-[-0.5px]">
              ...
              <button
                ...
                className="mt-2 h-[52px] w-full border-none bg-[#D6E264] font-grotesk text-[14px] font-normal text-[#0A0A0A] tracking-[1.5px] md:tracking-[2px] transition-colors cursor-pointer hover:bg-[#c9d64f] disabled:opacity-30 disabled:cursor-not-allowed"
              >
          ...
          {step === 2 && (
            <div className="flex flex-col gap-4">
              <div>
                ...
                <p className="font-ibm-mono text-[14px] text-[#666666] tracking-[0.5px] mt-1">
                  Review the package, then fill in what you can provide.
                </p>
              </div>

              {/* What's included */}
              <div className="flex flex-col gap-2 p-4 border border-[#D6E264] bg-white">
                <span className="font-ibm-mono text-[14px] font-normal text-[#6B7800] tracking-[1px] md:tracking-[2px]">WHAT&apos;S INCLUDED</span>
                {pkg.whatsIncluded.map((item) => (
                  <div key={item} className="flex items-start gap-2">
                    <span className="font-ibm-mono text-[14px] text-[#6B7800] mt-0.5 shrink-0">✓</span>
              ...
              {/* Fields */}
              <span className="font-ibm-mono text-[14px] font-normal text-[#777777] tracking-[1px] md:tracking-[2px]">WHAT WE NEED FROM YOU</span>
              ...
              <div className="mt-2 flex flex-col gap-[2px] sm:flex-row">
                <button
                  onClick={() => setStep(1)}
                  disabled={submitting}
                  className="h-[52px] border border-[#D8D8D8] bg-[#EFEFEF] px-6 font-ibm-mono text-[14px] text-[#666666] tracking-[1.5px] md:tracking-[2px] transition-colors cursor-pointer border-none hover:border-[#555]"
                >
                  ← BACK
                </button>
                <button
                  ...
                  disabled={submitting}
                  className="h-[52px] flex-1 border-none bg-[#D6E264] font-grotesk text-[14px] font-normal text-[#0A0A0A] tracking-[1.5px] md:tracking-[2px] transition-colors cursor-pointer hover:bg-[#c9d64f] disabled:opacity-40"
                >
          ...
          {step === 3 && (
            <div className="flex flex-col gap-4">
              ...
              {/* Order summary */}
              <div className="flex flex-col gap-3 p-5 bg-white border border-[#D8D8D8]">
                <div className="flex items-start justify-between pb-3 border-b border-[#E5E5E5]">
                  <div>
                    <span className="font-grotesk text-[16px] font-normal text-[#0A0A0A]">{pkg.icon} {pkg.name}</span>
                    <p className="font-ibm-mono text-[14px] text-[#666666] tracking-[1px] mt-1">DELIVERED IN {pkg.deliveryDays} DAYS</p>
                  </div>
                  <span className="shrink-0 font-grotesk text-[20px] sm:text-[22px] font-normal text-[#6B7800] tracking-[-1px]">£{pkg.price.toLocaleString()}</span>
                </div>
                <div>
                  <span className="font-ibm-mono text-[14px] text-[#777777] tracking-[2px]">ORDER FOR</span>
                  <p className="font-ibm-mono text-[14px] text-[#CCCCCC] mt-1">{s1.firstName} {s1.lastName}</p>
                  <p className="font-ibm-mono text-[14px] text-[#666666]">{s1.email}{s1.company ? ` · ${s1.company}` : ""}</p>
                </div>
              </div>

              {/* What happens next */}
              <div className="flex flex-col gap-2 p-4 bg-white border border-[#E5E5E5]">
                <span className="font-ibm-mono text-[14px] font-normal text-[#6B7800] tracking-[1px] md:tracking-[2px]">WHAT HAPPENS NEXT</span>
                {[
                  ...
                  <div key={i} className="flex items-start gap-2">
                    <span className="font-ibm-mono text-[14px] text-[#6B7800] shrink-0 mt-0.5">{`0${i + 1}`}</span>
