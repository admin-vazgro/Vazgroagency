            ...
            return (
              <div key={col.key}>
                <div className="mb-3 flex items-center justify-between">
                  <span className="font-ibm-mono text-[10px] tracking-[2px]" style={{ color: col.color }}>
                    {col.label}
                  </span>
                  <span className="font-ibm-mono text-[10px] text-[var(--portal-text-faint)]">{items.length}</span>
                </div>
                <div className="flex flex-col gap-3">
                    ...
                    return (
                      <div key={r.id} className="border border-[var(--portal-border)] bg-[var(--portal-surface)] p-4">
                        <div className="mb-2 flex items-center justify-between">
                          <span className="font-ibm-mono text-[9px] text-[var(--portal-text-dim)]">{r.id.slice(0, 8)}</span>
                          <span
                        ...
                        <p className="font-ibm-mono text-[11px] leading-[1.5] text-[var(--portal-text)] mb-2">{r.title}</p>
                        {r.description && (
                          <p className="font-ibm-mono text-[10px] leading-[1.6] text-[var(--portal-text-dim)] mb-2 line-clamp-2">
                            {r.description}
                          </p>
                        )}
                        {eng && (
                          <span className="font-ibm-mono text-[9px] text-[var(--portal-text-faint)]">{eng.title}</span>
                        )}
                        <p className="font-ibm-mono text-[9px] text-[var(--portal-text-faint)] mt-1">
                  ...
                  {items.length === 0 && (
                    <div className="border border-dashed border-[var(--portal-border)] p-6 text-center">
                      <p className="font-ibm-mono text-[10px] text-[var(--portal-text-faint)]">Empty</p>
                    </div>
                  )}
                </div>
              </div>
        ...
        <div className="border border-[var(--portal-border)] bg-[var(--portal-surface)]">
          ...
          {requests.map((r) => {
            ...
            return (
              ...
              >
                <span className="font-ibm-mono text-[10px] text-[var(--portal-text-dim)]">{r.id.slice(0, 8)}</span>
                <div>
                  <p className="font-ibm-mono text-[11px] text-[var(--portal-text)]">{r.title}</p>
                  {r.description && (
                    <p className="mt-0.5 font-ibm-mono text-[10px] leading-[1.5] text-[var(--portal-text-dim)] line-clamp-1">
                      {r.description}
                    </p>
                  )}
                </div>
                <span className="font-ibm-mono text-[10px] text-[var(--portal-text-soft)]">{eng?.title ?? "—"}</span>
                <span
                ...
                <span className="font-ibm-mono text-[10px]" style={{ color: colColor }}>
                  {colLabel}
                </span>
        ...
        >
          ...
          >
            <div className="border-b border-[var(--portal-border)] px-6 py-5">
              <span className="font-ibm-mono text-[10px] tracking-[3px] text-[var(--portal-accent)]">// NEW REQUEST</span>
            ...
            <form action={submitRequestAction} className="flex flex-col gap-4 p-6">
              <div>
                ...
                <input
                  ...
                  className="w-full border border-[var(--portal-border-strong)] bg-[var(--portal-bg)] px-4 py-3 font-ibm-mono text-[12px] text-[var(--portal-text)] placeholder:text-[var(--portal-text-faint)] focus:border-[var(--portal-accent)] focus:outline-none"
                />
              </div>
              <div>
                <label className="mb-2 block font-ibm-mono text-[10px] tracking-[2px] text-[var(--portal-text-soft)]">DETAILS</label>
                <textarea
                  name="description"
                  rows={4}
                  placeholder="Provide as much detail as possible..."
                  className="w-full resize-none border border-[var(--portal-border-strong)] bg-[var(--portal-bg)] px-4 py-3 font-ibm-mono text-[12px] text-[var(--portal-text)] placeholder:text-[var(--portal-text-faint)] focus:border-[var(--portal-accent)] focus:outline-none"
                />
              </div>
              <div>
                <label className="mb-2 block font-ibm-mono text-[10px] tracking-[2px] text-[var(--portal-text-soft)]">PROJECT *</label>
                <select
                  name="engagement_id"
                  required
                  defaultValue=""
              ...
              <div>
                <label className="mb-2 block font-ibm-mono text-[10px] tracking-[2px] text-[var(--portal-text-soft)]">PRIORITY</label>
                ...
                <div className="flex gap-2">
                  {(["low", "normal", "high"] as const).map((p) => (
                    <button
                      ...
                      className="flex-1 border py-2 font-ibm-mono text-[10px] tracking-[1px] transition-colors cursor-pointer"
                      style={{
              ...
              <div className="flex gap-3 pt-2">
                <button
                  type="button"
                  onClick={() => setShowNew(false)}
                  className="flex-1 border border-[var(--portal-border-strong)] bg-transparent py-3 font-ibm-mono text-[10px] tracking-[2px] text-[var(--portal-text-muted)] transition-colors hover:text-[var(--portal-text)] cursor-pointer"
                >
                  CANCEL
                </button>
                <button
                  type="submit"
                  className="flex-1 border-none bg-[var(--portal-accent)] py-3 font-ibm-mono text-[10px] tracking-[2px] text-[var(--portal-accent-contrast)] transition-colors hover:bg-[var(--portal-accent-hover)] cursor-pointer"
                >
                  SUBMIT REQUEST →
                </button>
              </div>
