  ...
  return (
    <div className="p-8">
      <div className="mb-8 border-b border-[var(--portal-border)] pb-6">
        <span className="font-ibm-mono text-[10px] tracking-[3px] text-[var(--portal-accent)]">// EARNINGS</span>
      ...
      <div className="mb-8 grid grid-cols-1 gap-4 lg:grid-cols-3">
        <div className="border border-[var(--portal-border)] bg-[var(--portal-surface)] p-5">
          <p className="font-ibm-mono text-[10px] tracking-[2px] text-[var(--portal-text-muted)]">PENDING</p>
          <p className="mt-2 font-grotesk text-[30px] font-normal text-[var(--portal-warning)]">{formatMoney(pendingTotal)}</p>
          <p className="mt-1 font-ibm-mono text-[10px] text-[var(--portal-text-dim)]">Inside claw-back hold</p>
        </div>
        <div className="border border-[var(--portal-border)] bg-[var(--portal-surface)] p-5">
          <p className="font-ibm-mono text-[10px] tracking-[2px] text-[var(--portal-text-muted)]">APPROVED</p>
          <p className="mt-2 font-grotesk text-[30px] font-normal text-[var(--portal-accent)]">{formatMoney(approvedTotal)}</p>
          <p className="mt-1 font-ibm-mono text-[10px] text-[var(--portal-text-dim)]">Awaiting payout run</p>
        </div>
        <div className="border border-[var(--portal-border)] bg-[var(--portal-surface)] p-5">
          <p className="font-ibm-mono text-[10px] tracking-[2px] text-[var(--portal-text-muted)]">PAID LIFETIME</p>
          <p className="mt-2 font-grotesk text-[30px] font-normal text-[var(--portal-text)]">{formatMoney(paidLifetime)}</p>
          <p className="mt-1 font-ibm-mono text-[10px] text-[var(--portal-text-dim)]">Total cash released</p>
        </div>
      </div>
      ...
      <form method="get" className="mb-6 flex flex-wrap items-end gap-3">
        <div>
          <label className="mb-2 block font-ibm-mono text-[9px] tracking-[2px] text-[var(--portal-text-dim)]">STATUS</label>
          <select name="status" defaultValue={statusFilter} className="border border-[var(--portal-border)] bg-[var(--portal-surface-alt)] px-3 py-2 font-ibm-mono text-[10px] text-[var(--portal-text)]">
        ...
        <div>
          <label className="mb-2 block font-ibm-mono text-[9px] tracking-[2px] text-[var(--portal-text-dim)]">PILLAR</label>
          <select name="pillar" defaultValue={pillarFilter} className="border border-[var(--portal-border)] bg-[var(--portal-surface-alt)] px-3 py-2 font-ibm-mono text-[10px] text-[var(--portal-text)]">
            <option value="all">ALL</option>
        ...
        <div>
          <label className="mb-2 block font-ibm-mono text-[9px] tracking-[2px] text-[var(--portal-text-dim)]">ROLE</label>
          <select name="role" defaultValue={roleFilter} className="border border-[var(--portal-border)] bg-[var(--portal-surface-alt)] px-3 py-2 font-ibm-mono text-[10px] text-[var(--portal-text)]">
            <option value="all">ALL</option>
            <option value="closer">CLOSER</option>
            <option value="referrer">REFERRER</option>
          </select>
        </div>
        <button type="submit" className="border border-[var(--portal-border-strong)] px-4 py-2 font-ibm-mono text-[10px] tracking-[2px] text-[var(--portal-text-soft)]">
          APPLY
        </button>
      </form>
      ...
      <div className="border border-[var(--portal-border)] bg-[var(--portal-surface)]">
        ...
        {filtered.map((row) => (
          <div key={row.id} className="grid grid-cols-[80px_1fr_80px_80px_120px_100px_130px_130px] gap-4 border-b border-[var(--portal-border)] px-5 py-4 hover:bg-[var(--portal-surface-alt)]">
            <span className="font-ibm-mono text-[10px] text-[var(--portal-text-dim)]">{row.id.slice(0, 8)}</span>
            <div>
              <p className="font-ibm-mono text-[11px] text-[var(--portal-text)]">{dealById.get(row.deal_id ?? "")?.title || "Linked deal"}</p>
              {taperCopy(row) ? (
                <p className="mt-1 font-ibm-mono text-[10px] text-[var(--portal-text-dim)]">{taperCopy(row)}</p>
              ) : null}
            </div>
            <span className="font-ibm-mono text-[10px] text-[var(--portal-text-soft)]">{row.pillar ?? "—"}</span>
            <span className="font-ibm-mono text-[10px] text-[var(--portal-text-soft)]">{row.recipient_type ?? "—"}</span>
            <span className="font-ibm-mono text-[10px] text-[var(--portal-text-soft)]">
              {row.rate_percent}% · {row.tier}
            </span>
            <span className="font-ibm-mono text-[10px] text-[var(--portal-text)]">{formatMoney(row.commission_amount_gbp)}</span>
            <div>
              <p className="font-ibm-mono text-[10px] text-[var(--portal-text-soft)]">{row.status}</p>
              {row.claw_back_window_ends ? (
                <p className="mt-1 font-ibm-mono text-[9px] text-[var(--portal-text-dim)]">
                  Hold ends {new Date(row.claw_back_window_ends).toLocaleDateString("en-GB")}
                </p>
              ) : null}
            </div>
            <div>
              <p className="font-ibm-mono text-[10px] text-[var(--portal-text-soft)]">
                {row.paid_at ? new Date(row.paid_at).toLocaleDateString("en-GB") : "Not paid"}
              </p>
              {row.statement_pdf_url ? (
                <a href={row.statement_pdf_url} target="_blank" rel="noreferrer" className="mt-1 inline-block font-ibm-mono text-[9px] text-[var(--portal-accent)]">
                  STATEMENT →
                </a>
              ) : null}
            </div>
        ...
        {!filtered.length ? (
          <div className="px-5 py-12">
            <p className="font-ibm-mono text-[11px] text-[var(--portal-text-soft)]">No commissions in this view yet.</p>
          </div>
        ) : null}
      </div>
    </div>
