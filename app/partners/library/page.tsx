  ...
  return (
    <div className="p-8">
      <div className="mb-8 border-b border-[var(--portal-border)] pb-6">
        <span className="font-ibm-mono text-[14px] tracking-[3px] text-[var(--portal-accent)]">// SALES LIBRARY</span>
      ...
      <form method="get" className="mb-6 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div className="flex flex-1 flex-col gap-2">
          <label className="font-ibm-mono text-[14px] tracking-[2px] text-[var(--portal-text-dim)]">SEARCH</label>
          <input
            type="text"
            name="q"
            defaultValue={query}
            placeholder="Pitch deck, pricing, case study..."
            className="w-full border border-[var(--portal-border-strong)] bg-[var(--portal-surface)] px-4 py-3 font-ibm-mono text-[14px] text-[var(--portal-text)] placeholder:text-[var(--portal-text-dim)] focus:border-[var(--portal-accent)] focus:outline-none"
          />
        </div>
        <div className="flex flex-col gap-2">
          <label className="font-ibm-mono text-[14px] tracking-[2px] text-[var(--portal-text-dim)]">CATEGORY</label>
          <select name="category" defaultValue={category} className="border border-[var(--portal-border-strong)] bg-[var(--portal-surface)] px-4 py-3 font-ibm-mono text-[14px] text-[var(--portal-text)] focus:border-[var(--portal-accent)] focus:outline-none">
        ...
        <button type="submit" className="border border-[var(--portal-border-strong)] px-4 py-3 font-ibm-mono text-[14px] tracking-[2px] text-[var(--portal-text-soft)]">
          APPLY
        </button>
      </form>
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
        {filtered.map((item) => (
          <div key={item.id} className="border border-[var(--portal-border)] bg-[var(--portal-surface)] p-5">
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="font-ibm-mono text-[14px] tracking-[2px] text-[var(--portal-accent)]">{categoryLabel(item.category).toUpperCase()}</p>
            ...
            {item.description ? (
              <p className="mt-4 font-ibm-mono text-[14px] leading-[1.7] text-[var(--portal-text-soft)]">{item.description}</p>
            ) : null}
            <div className="mt-5 flex flex-wrap items-center gap-3">
              {item.file_url ? (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="bg-[var(--portal-accent)] px-4 py-3 font-ibm-mono text-[14px] tracking-[2px] text-[var(--portal-accent-contrast)]"
                >
                  OPEN ASSET
                </a>
              ) : null}
              {item.thumbnail_url ? (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="border border-[var(--portal-border-strong)] px-4 py-3 font-ibm-mono text-[14px] tracking-[2px] text-[var(--portal-text-soft)]"
                >
                  PREVIEW
                </a>
              ) : null}
        ...
        {!filtered.length ? (
          <div className="border border-[var(--portal-border)] bg-[var(--portal-surface)] px-6 py-10 xl:col-span-2">
            <p className="font-ibm-mono text-[14px] text-[var(--portal-text-soft)]">No partner-facing assets match the current filter.</p>
          </div>
        ) : null}
      </div>
    </div>
