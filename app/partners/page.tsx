  ...
  return (
    <div className="p-8">
      <div className="mb-8 border-b border-[var(--portal-border)] pb-6">
        <span className="font-ibm-mono text-[14px] tracking-[3px] text-[var(--portal-accent)]">// PARTNER PORTAL</span>
      ...
      {needsOnboarding ? (
        <div className="mb-6 border border-[var(--portal-warning)] bg-[var(--portal-warning-soft)] px-5 py-4 flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-2 font-ibm-mono text-[14px] leading-[1.7] text-[var(--portal-text-soft)]">
      ...
      <div className="mb-8 grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-5">
        <div className="border border-[var(--portal-border)] bg-[var(--portal-surface)] p-5">
          <p className="font-ibm-mono text-[14px] tracking-[2px] text-[var(--portal-text-muted)]">CLOSED THIS MONTH</p>
          <p className="mt-2 font-grotesk text-[30px] font-normal text-[var(--portal-accent)]">{formatMoney(monthlyRevenue)}</p>
        </div>
        <div className="border border-[var(--portal-border)] bg-[var(--portal-surface)] p-5">
          <p className="font-ibm-mono text-[14px] tracking-[2px] text-[var(--portal-text-muted)]">CURRENT TIER</p>
          <p className="mt-2 font-grotesk text-[30px] font-normal text-[var(--portal-text)]">{currentTierLabel}</p>
          <p className="mt-1 font-ibm-mono text-[14px] text-[var(--portal-text-dim)]">{currentCloserRate}% closer rate</p>
        </div>
        <div className="border border-[var(--portal-border)] bg-[var(--portal-surface)] p-5">
          <p className="font-ibm-mono text-[14px] tracking-[2px] text-[var(--portal-text-muted)]">COMMISSIONS PENDING</p>
          <p className="mt-2 font-grotesk text-[30px] font-normal text-[var(--portal-warning)]">{formatMoney(pendingTotal)}</p>
        </div>
        <div className="border border-[var(--portal-border)] bg-[var(--portal-surface)] p-5">
          <p className="font-ibm-mono text-[14px] tracking-[2px] text-[var(--portal-text-muted)]">PAID THIS MONTH</p>
          <p className="mt-2 font-grotesk text-[30px] font-normal text-[var(--portal-text)]">{formatMoney(paidThisMonth)}</p>
        </div>
        <div className="border border-[var(--portal-border)] bg-[var(--portal-surface)] p-5">
          <p className="font-ibm-mono text-[14px] tracking-[2px] text-[var(--portal-text-muted)]">LEAD CAP</p>
          <p className="mt-2 font-grotesk text-[30px] font-normal text-[var(--portal-text)]">{openLeadCount}/{cap}</p>
          <p className="mt-1 font-ibm-mono text-[14px] text-[var(--portal-text-dim)]">{canClaimFromPool ? "Pool claim open" : "Pool claim limited"}</p>
        </div>
      </div>
      ...
      {tierProgress ? (
        <div className="mb-8 border border-[var(--portal-border)] bg-[var(--portal-surface)] p-5">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            <span className="font-ibm-mono text-[14px] text-[var(--portal-text-dim)]">{tierProgress.current.label}</span>
            <span className="font-ibm-mono text-[14px] text-[var(--portal-text-dim)]">Closed revenue this month: {formatMoney(monthlyRevenue)}</span>
          </div>
        </div>
      ) : null}
      ...
      <div className="mb-8 grid grid-cols-1 gap-6 xl:grid-cols-[1.1fr_0.9fr]">
        <div className="border border-[var(--portal-border)] bg-[var(--portal-surface)]">
          <div className="border-b border-[var(--portal-border)] px-6 py-4">
            <p className="font-ibm-mono text-[14px] tracking-[2px] text-[var(--portal-warning)]">NEEDS YOUR ATTENTION</p>
          </div>
          <div className="divide-y divide-[var(--portal-border)]">
            {atRiskLeads.map((lead) => {
              ...
              return (
                <Link key={lead.id} href={`/partners/leads?lead=${lead.id}`} className="flex items-start justify-between gap-4 px-6 py-4 hover:bg-[var(--portal-surface-alt)]">
                  <div>
                    <p className="font-ibm-mono text-[14px] text-[var(--portal-text)]">{displayLeadName(lead)}</p>
                    <p className="mt-1 font-ibm-mono text-[14px] text-[var(--portal-text-dim)]">
                      {lead.stage.replace(/_/g, " ")} · {lead.pillar ?? "—"}
                    </p>
                  </div>
                  <span className="font-ibm-mono text-[14px]" style={{ color: slaColor(hoursLeft) }}>
            ...
            {!atRiskLeads.length ? (
              <div className="px-6 py-6">
                <p className="font-ibm-mono text-[14px] text-[var(--portal-accent)]">No SLA warnings right now.</p>
              </div>
            ) : null}
          </div>
        </div>
        ...
        <div className="border border-[var(--portal-border)] bg-[var(--portal-surface)]">
          <div className="border-b border-[var(--portal-border)] px-6 py-4">
            <p className="font-ibm-mono text-[14px] tracking-[2px] text-[var(--portal-text)]">LEADERBOARD</p>
          </div>
          <div className="space-y-4 px-6 py-5">
            <div>
              <p className="font-ibm-mono text-[14px] text-[var(--portal-text-dim)]">Your rank this month</p>
            ...
            <div className="space-y-3">
              {above ? (
                <div className="border border-[var(--portal-border)] bg-[var(--portal-surface-alt)] px-4 py-3">
                  <p className="font-ibm-mono text-[14px] text-[var(--portal-text-dim)]">Above you</p>
                  <p className="mt-1 font-ibm-mono text-[14px] text-[var(--portal-text)]">{above.name}</p>
                  <p className="mt-1 font-ibm-mono text-[14px] text-[var(--portal-text-dim)]">{formatMoney(above.revenue)}</p>
                </div>
              ) : null}
              {below ? (
                <div className="border border-[var(--portal-border)] bg-[var(--portal-surface-alt)] px-4 py-3">
                  <p className="font-ibm-mono text-[14px] text-[var(--portal-text-dim)]">Below you</p>
                  <p className="mt-1 font-ibm-mono text-[14px] text-[var(--portal-text)]">{below.name}</p>
                  <p className="mt-1 font-ibm-mono text-[14px] text-[var(--portal-text-dim)]">{formatMoney(below.revenue)}</p>
                </div>
              ) : null}
              {!above && !below ? (
                <p className="font-ibm-mono text-[14px] text-[var(--portal-text-dim)]">Leaderboard will populate once there are multiple active partners this month.</p>
              ) : null}
            </div>
          </div>
        </div>
      ...
      <div className="mb-8 grid grid-cols-1 gap-6 xl:grid-cols-[1.1fr_0.9fr]">
        <div className="border border-[var(--portal-border)] bg-[var(--portal-surface)]">
          <div className="flex items-center justify-between border-b border-[var(--portal-border)] px-6 py-4">
            <p className="font-ibm-mono text-[14px] tracking-[2px] text-[var(--portal-text)]">ACTIVE DEALS</p>
            <Link href="/partners/deals" className="font-ibm-mono text-[14px] text-[var(--portal-accent)]">VIEW ALL →</Link>
          </div>
          <div className="divide-y divide-[var(--portal-border)]">
            ...
            {!activeDeals.length ? (
              <div className="px-6 py-6">
                <p className="font-ibm-mono text-[14px] text-[var(--portal-text-soft)]">No active deals yet.</p>
              </div>
            ) : null}
          </div>
        </div>
        ...
        <div className="border border-[var(--portal-border)] bg-[var(--portal-surface)]">
          <div className="border-b border-[var(--portal-border)] px-6 py-4">
            <p className="font-ibm-mono text-[14px] tracking-[2px] text-[var(--portal-text)]">PRIMARY ACTIONS</p>
          </div>
          <div className="grid grid-cols-1 gap-3 p-6">
            <Link href="/partners/leads?new=1" className="flex items-center justify-between border border-[var(--portal-border)] bg-[var(--portal-surface-alt)] p-4">
              <div>
                <p className="font-ibm-mono text-[14px] tracking-[2px] text-[var(--portal-accent)]">+ SUBMIT A LEAD</p>
                <p className="mt-1 font-ibm-mono text-[14px] text-[var(--portal-text-dim)]">Refer a new prospect and lock attribution.</p>
              </div>
            ...
            <Link href="/partners/leads#lead-pool" className="flex items-center justify-between border border-[var(--portal-border)] bg-[var(--portal-surface-alt)] p-4">
              <div>
                <p className="font-ibm-mono text-[14px] tracking-[2px] text-[var(--portal-text)]">CLAIM FROM POOL</p>
                <p className="mt-1 font-ibm-mono text-[14px] text-[var(--portal-text-dim)]">
            ...
            <Link href={activeDeals[0] ? `/partners/deals?deal=${activeDeals[0].id}` : "/partners/deals"} className="flex items-center justify-between border border-[var(--portal-border)] bg-[var(--portal-surface-alt)] p-4">
              <div>
                <p className="font-ibm-mono text-[14px] tracking-[2px] text-[var(--portal-text)]">OPEN MY NEXT DEAL</p>
                <p className="mt-1 font-ibm-mono text-[14px] text-[var(--portal-text-dim)]">Jump back into the next live opportunity.</p>
              </div>
              <span className="font-ibm-mono text-[18px] text-[var(--portal-text-dim)]">→</span>
            </Link>
          </div>
