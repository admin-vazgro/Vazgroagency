...
export default async function HubRequestsPage(props: {
  ...
  const inputClass =
    "w-full border border-[var(--portal-border-strong)] bg-[var(--portal-bg)] px-3 py-2 font-ibm-mono text-[14px] text-[var(--portal-text)] focus:border-[var(--portal-accent)] focus:outline-none";
    ...
    <div className="p-8">
        ...
        {["all", ...STATUS_ORDER].map((s) => {
          ...
          return (
            <Link
              ...
              className="px-3 py-1.5 font-ibm-mono text-[14px] tracking-[1px] border transition-colors"
              style={{
      ...
      {filtered.length === 0 && !dataError ? (
        <div className="border border-dashed border-[var(--portal-border-strong)] p-8">
          <p className="font-ibm-mono text-[14px] text-[var(--portal-text-soft)]">No requests in this view.</p>
          ...
          <div className="border border-[var(--portal-border)] bg-[var(--portal-surface)]">
            ...
            {filtered.map((req) => {
              ...
              return (
                ...
                >
                  <div>
                    <p className="font-ibm-mono text-[14px] text-[var(--portal-text)] leading-[1.4]">{req.title}</p>
                    <p className="mt-0.5 font-ibm-mono text-[14px] text-[var(--portal-text-dim)]">
                      {assignee ? `→ ${displayName(assignee)}` : "Unassigned"}
                    </p>
                  </div>
                  <div>
                    <p className="font-ibm-mono text-[14px] text-[var(--portal-text-soft)]">{acc?.name ?? "—"}</p>
                    <p className="mt-0.5 font-ibm-mono text-[14px] text-[var(--portal-text-dim)]">{eng?.title ?? "—"}</p>
                  </div>
                  <span
                    className="font-ibm-mono text-[14px] self-center"
                  ...
                  <span className="font-ibm-mono text-[14px] text-[var(--portal-accent)] self-center">OPEN →</span>
                </Link>
              );
            })}
          </div>
            ...
            <div className="flex flex-col gap-4">
              <div className="border border-[var(--portal-border)] bg-[var(--portal-surface)]">
                <div className="border-b border-[var(--portal-border)] px-5 py-4">
                  <p className="font-ibm-mono text-[14px] tracking-[2px] text-[var(--portal-text)]">REQUEST DETAIL</p>
                ...
                <div className="px-5 py-4 grid grid-cols-2 gap-3">
                  {[
                    ...
                    <div key={label}>
                      <p className="font-ibm-mono text-[14px] tracking-[1.5px] text-[var(--portal-text-dim)]">{label.toUpperCase()}</p>
                      <p className="mt-1 font-ibm-mono text-[14px]" style={{ color }}>{value}</p>
                    </div>
                  ))}
                </div>
                ...
                {selectedRequest.description && (
                  <div className="border-t border-[var(--portal-border)] px-5 py-4">
                    <p className="font-ibm-mono text-[14px] tracking-[2px] text-[var(--portal-text-dim)] mb-2">DESCRIPTION</p>
                ...
                {selectedRequest.revision_notes && (
                  <div className="border-t border-[var(--portal-warning)] bg-[var(--portal-warning-soft)] px-5 py-4">
                    <p className="font-ibm-mono text-[14px] tracking-[2px] text-[var(--portal-warning)] mb-2">REVISION NOTES</p>
                    <p className="font-ibm-mono text-[14px] leading-[1.7] text-[var(--portal-text-soft)]">
                      {selectedRequest.revision_notes}
                    </p>
                  </div>
              ...
              <div className="border border-[var(--portal-border)] bg-[var(--portal-surface)] p-5">
                <p className="font-ibm-mono text-[14px] tracking-[2px] text-[var(--portal-text)] mb-4">UPDATE REQUEST</p>
                <form action={updateRequestStatusAction} className="flex flex-col gap-3">
                  ...
                  <div>
                    <label className="mb-1.5 block font-ibm-mono text-[14px] tracking-[2px] text-[var(--portal-text-dim)]">
                      STATUS
                    </label>
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      className="w-full resize-y border border-[var(--portal-border-strong)] bg-[var(--portal-bg)] px-3 py-2 font-ibm-mono text-[14px] text-[var(--portal-text)] focus:border-[var(--portal-accent)] focus:outline-none"
                  ...
                  <button
                    type="submit"
                    className="bg-[var(--portal-accent)] px-5 py-3 font-ibm-mono text-[14px] tracking-[2px] text-[var(--portal-accent-contrast)] hover:bg-[var(--portal-accent-hover)] transition-colors"
                  >
                    UPDATE REQUEST →
                  </button>
                </form>
              ...
              {selectedRequest.account_id && (
                <div className="border border-[var(--portal-border)] bg-[var(--portal-surface)] px-5 py-4">
                  <p className="font-ibm-mono text-[14px] tracking-[2px] text-[var(--portal-text-dim)] mb-2">RELATED</p>
                  <Link
                    href={`/hub/engagements`}
                    className="font-ibm-mono text-[14px] text-[var(--portal-accent)] hover:opacity-80"
                  >
                    View Engagements →
                  </Link>
                </div>
