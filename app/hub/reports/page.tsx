  ...
  return (
    <div className="p-8">
      <div className="mb-8 border-b border-[var(--portal-border)] pb-6">
        <span className="font-ibm-mono text-[14px] text-[var(--portal-accent)] tracking-[3px]">// REPORTS</span>
        <h1 className="font-grotesk text-[32px] font-bold text-[var(--portal-text)] tracking-[-1px] mt-1">Reports & Analytics</h1>
        <p className="font-ibm-mono text-[14px] text-[var(--portal-text-soft)] tracking-[0.5px] mt-1">Rolling 6-month view · Last updated 10 Apr 2026</p>
      </div>
      ...
      <div className="grid grid-cols-[1fr_280px] gap-6 mb-6">
        ...
        <div className="border border-[var(--portal-border)] bg-[var(--portal-surface)] p-6">
          <p className="font-ibm-mono text-[14px] text-[var(--portal-text)] tracking-[2px] mb-6">MONTHLY REVENUE</p>
          {months.length ? (
            <div className="flex items-end gap-4 h-40">
              {months.map((m, i) => (
              <div key={m} className="flex-1 flex flex-col items-center gap-2">
                <span className="font-ibm-mono text-[14px] text-[var(--portal-text-soft)]">£{(revenue[i] / 1000).toFixed(1)}k</span>
                <div
                ...
                />
                <span className="font-ibm-mono text-[14px] text-[var(--portal-text-dim)]">{m}</span>
              </div>
              ))}
            </div>
          ) : (
            <div className="py-12">
              <p className="font-ibm-mono text-[14px] text-[var(--portal-text-soft)]">No revenue data yet.</p>
            </div>
          )}
        </div>
        ...
        {/* Pillar breakdown */}
        <div className="border border-[var(--portal-border)] bg-[var(--portal-surface)] p-6">
          <p className="font-ibm-mono text-[14px] text-[var(--portal-text)] tracking-[2px] mb-6">PILLAR BREAKDOWN (6MO)</p>
              ...
              return (
                <div key={p.pillar}>
                  <div className="flex items-center justify-between mb-1.5">
                    <span className="font-ibm-mono text-[14px]" style={{ color: p.color }}>{p.pillar}</span>
                    <span className="font-ibm-mono text-[14px] text-[var(--portal-text-muted)]">£{p.revenue.toLocaleString()}</span>
                  </div>
                  <div className="w-full bg-[var(--portal-border)] h-2">
                    <div className="h-2" style={{ width: `${pct}%`, background: p.color }} />
                  </div>
                  <p className="font-ibm-mono text-[14px] text-[var(--portal-text-dim)] mt-1">{p.deals} deals · {pct}%</p>
                </div>
              );
            }) : (
              <p className="font-ibm-mono text-[14px] text-[var(--portal-text-soft)]">No pillar data yet.</p>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="border border-[var(--portal-border)] bg-[var(--portal-surface)]">
        <div className="px-6 py-4 border-b border-[var(--portal-border)]">
          <span className="font-ibm-mono text-[14px] text-[var(--portal-text)] tracking-[2px]">PARTNER PERFORMANCE (6MO)</span>
        </div>
        ...
        {topPartners.map((p) => (
          <div key={p.name} className="grid grid-cols-[1fr_80px_120px_120px] gap-4 px-5 py-4 border-b border-[var(--portal-border)] hover:bg-[var(--portal-surface-alt)] transition-colors">
            <span className="font-ibm-mono text-[14px] text-[var(--portal-text-muted)]">{p.name}</span>
            <span className="font-ibm-mono text-[14px] text-[var(--portal-text-muted)]">{p.closed}</span>
            <span className="font-ibm-mono text-[14px] text-[var(--portal-text-muted)]">£{p.revenue.toLocaleString()}</span>
            <span className="font-ibm-mono text-[14px] text-[var(--portal-accent)]">£{p.commission.toLocaleString()}</span>
          </div>
        ))}
        {topPartners.length === 0 && (
          <div className="px-5 py-12">
            <p className="font-ibm-mono text-[14px] text-[var(--portal-text-soft)]">No partner performance data yet.</p>
          </div>
        )}
      </div>
    </div>
